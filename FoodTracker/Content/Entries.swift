//
//  Entries.swift
//  FoodTracker
//
//  Created by Jeff Braun on 04.08.25.
//

import Foundation

let entriesList: [Entry] = [
    Entry(title: "Breakfast - Oatmeal", date: Date(), calories: 350),
    Entry(title: "Morning Snack - Banana", date: Date(), calories: 90),
    Entry(title: "Lunch - Grilled Chicken Salad", date: Date(), calories: 450),
    Entry(title: "Afternoon Snack - Almonds", date: Date(), calories: 180),
    Entry(title: "Dinner - Spaghetti Bolognese", date: Date(), calories: 700),
    Entry(title: "Evening Snack - Yogurt", date: Date(), calories: 120),
    Entry(title: "Smoothie", date: Date(), calories: 200),
    Entry(title: "Energy Bar", date: Date(), calories: 150),
    Entry(title: "Pizza Slice", date: Date(), calories: 300),
    Entry(title: "Ice Cream", date: Date(), calories: 250)
]

let mealsList: [Entry] = [
    Entry(title: "Breakfast - Oatmeal", calories: 350),
    Entry(title: "Lunch - Chicken Salad", calories: 500),
    Entry(title: "Dinner - Pasta", calories: 650)
]

let snacksList: [Entry] = [
    Entry(title: "Banana", calories: 90),
    Entry(title: "Granola Bar", calories: 150),
    Entry(title: "Yogurt", calories: 120)
]

let drinksList: [Entry] = [
    Entry(title: "Coffee", calories: 60),
    Entry(title: "Smoothie", calories: 200),
    Entry(title: "Orange Juice", calories: 110)
]
